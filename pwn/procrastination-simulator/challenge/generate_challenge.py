...
for round_number in range(N, 0, -1):
    ...
    if round_number > level_three_threshold: # level 4
        generate_level_four_source_code(filename_stem="binary_{}".format(round_number), password=password) # No random seed for this level, we use ASLR and PIE
        seed = int(powmod(mpz(seed),234234,120987234))
        ...
        generate_level_three_source_code(filename_stem="binary_{}".format(round_number), password=password, random_seed=seed) # Source code is the same
        seed = int(powmod(mpz(seed),234234,120987234))
